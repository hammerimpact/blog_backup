...
# 고해상도 수정(Highres. fix)
이미지를 저해상도에서 부분적으로 렌더링하고 확대한 다음 고해상도로 세부정보를 추가할 수 있는 편리한 옵션입니다. 기본적으로 txt2img는 매우 높은 해상도에서 끔찍한 이미지를 생성하므로 작은 그림의 구도를 사용하는 것을 피할 수 있습니다. txt2img 페이지에서 "고해상도 수정" 확인란을 선택하여 활성화합니다.
...
--lowvrambasujindal 의 최적화 아이디어를 다시 구현한 것 입니다. 모델은 모듈로 분리되며 GPU 메모리에는 하나의 모듈만 보관됩니다. 다른 모듈을 실행해야 하는 경우 이전 모듈이 GPU 메모리에서 제거됩니다. 이 최적화의 특성으로 인해 처리가 느리게 실행됩니다. RTX 3090에서 일반 작업에 비해 약 10배 더 느립니다.

--medvram동일한 배치에서 조건부 잡음 제거 및 무조건 잡음 제거를 처리하지 않음으로써 VRAM 사용량을 크게 줄여야 하는 또 다른 최적화입니다.

이 최적화 구현에서는 원본 Stable Diffusion 코드를 수정할 필요가 없습니다.

얼굴 복원
GFPGAN 또는 CodeFormer를 사용하여 사진의 얼굴을 개선할 수 있습니다. 모든 탭에는 얼굴 복원을 사용할 수 있는 확인란이 있으며 효과의 가시성을 제어하는 ​​슬라이더가 있는 모든 사진에서 얼굴 복원을 사용할 수 있는 별도의 탭도 있습니다. 설정에서 두 가지 방법 중에서 선택할 수 있습니다.

![](2022-10-20-13-11-47.png)

절약
출력 섹션에서 저장 버튼을 클릭하면 생성된 이미지가 설정에 지정된 디렉토리에 저장됩니다. 생성 매개변수는 동일한 디렉토리의 csv 파일에 추가됩니다.

로딩 중
Gradio의 로딩 그래픽은 신경망의 처리 속도에 매우 부정적인 영향을 미칩니다. 내 RTX 3090은 그라디오가 있는 탭이 활성화되어 있지 않을 때 이미지를 약 10% 더 빠르게 만듭니다. 기본적으로 UI는 이제 로딩 진행 애니메이션을 숨기고 정적 "로드 중..." 텍스트로 대체하여 동일한 효과를 얻습니다. 명령줄 옵션을 사용하여 --no-progressbar-hiding이를 되돌리고 로딩 애니메이션을 표시합니다.

신속한 검증
안정적인 확산은 입력 텍스트 길이에 제한이 있습니다. 프롬프트가 너무 길면 텍스트 출력 필드에 경고가 표시되어 모델에서 잘리고 무시된 텍스트 부분을 보여줍니다.

PNG 정보
생성 매개변수에 대한 정보를 PNG에 텍스트 청크로 추가합니다. 이 정보는 나중에 PNG 청크 정보 보기를 지원하는 소프트웨어를 사용하여 볼 수 있습니다. 예: https://www.nayuki.io/page/png-file-chunk-inspector

설정
설정이 있는 탭에서는 UI를 사용하여 이전에 명령줄이었던 매개변수의 절반 이상을 편집할 수 있습니다. 설정은 config.js 파일에 저장됩니다. 명령줄 옵션으로 남아 있는 설정은 시작 시 필요한 설정입니다.

파일 이름 형식
설정 탭 의 Images filename pattern필드에서 생성된 txt2img 및 img2img 이미지 파일 이름을 사용자 지정할 수 있습니다. 이 패턴은 파일 이름과 그 순서에 포함할 생성 매개변수를 정의합니다. 지원되는 태그는 다음과 같습니다.
...
이 목록은 새로운 추가 사항과 함께 발전할 것입니다. UI에서 "이미지 파일 이름 패턴" 레이블 위로 마우스를 가져가면 지원되는 태그의 최신 목록을 얻을 수 있습니다.

패턴의 예:[seed]-[steps]-[cfg]-[sampler]-[prompt_spaces]

"prompt" 태그에 대한 참고 사항: [prompt]프롬프트 단어 사이에 밑줄을 추가하는 반면 [prompt_spaces]프롬프트는 그대로 유지합니다(UI에 다시 복사/붙여넣기가 더 쉬움). [prompt_words]프롬프트의 단어만 포함된(구두점 없음) 하위 디렉토리 이름을 생성하는 데 이미 사용된 프롬프트의 단순화되고 정리된 버전입니다.

이 필드를 비워두면 기본 패턴이 적용됩니다( [seed]-[prompt_spaces]).

태그는 실제로 패턴 내부에서 교체된다는 점에 유의하십시오. 이는 이 패턴에 태그가 아닌 단어를 추가하여 파일 이름을 더욱 명시적으로 만들 수도 있음을 의미합니다. 예를 들어:s=[seed],p=[prompt_spaces]

사용자 스크립트
프로그램이 --allow-code옵션과 함께 실행되면 페이지 하단의 스크립트 -> 사용자 정의 코드에서 스크립트 코드에 대한 추가 텍스트 입력 필드를 사용할 수 있습니다. 이미지 작업을 수행할 파이썬 코드를 입력할 수 있습니다.

코드에서 변수를 사용하여 웹 UI의 매개변수에 액세스 하고 함수 p를 사용하여 웹 UI에 대한 출력을 제공 display(images, seed, info)합니다. 스크립트의 모든 전역에도 액세스할 수 있습니다.
...
체크박스: 체크된 상태
텍스트 및 숫자 입력: 기본값
파일은 webui dir에 ui-config.json이며, 프로그램 시작 시 파일이 없으면 자동으로 생성됩니다.

일반적으로 숨겨진 섹션을 확장하는 확인란은 UI 구성 항목으로 설정된 경우 처음에는 그렇게 하지 않습니다.

너비와 높이에 대해 64로 나눌 수 없는 단계와 같은 일부 설정은 처리를 중단하고 img2img 탭의 기본 기능을 변경하는 것과 같은 일부 설정은 UI를 중단할 수 있습니다. 나는 가까운 장래에 그것들을 다룰 계획이 없습니다.

에스르간
Extras 탭과 SD upscale에서 ESRGAN 모델을 사용할 수 있습니다.

ESRGAN 모델을 사용하려면 webui.py와 같은 위치의 ESRGAN 디렉토리에 넣습니다. 확장자가 .pth인 파일은 모델로 로드됩니다. 모델 데이터베이스 에서 모델을 가져옵니다 .

데이터베이스의 모든 모델이 지원되는 것은 아닙니다. 모든 2x 모델은 대부분 지원되지 않습니다.

img2img 대체 테스트
오일러 디퓨저의 역순을 사용하여 입력 이미지를 분해하여 입력 프롬프트를 구성하는 데 사용되는 노이즈 패턴을 만듭니다.

예를 들어 이 이미지를 사용할 수 있습니다. 스크립트 섹션 에서 img2img 대체 테스트를 선택합니다 .

![](2022-10-20-13-11-55.png)

재구성 프로세스에 대한 설정을 조정합니다.

장면에 대한 간략한 설명을 사용하십시오. "갈색 머리를 한 웃는 여자." 변경하려는 기능을 설명하면 도움이 됩니다. 이것을 시작 프롬프트로 설정하고 스크립트 설정에서 '원래 입력 프롬프트'로 설정하십시오.
이 스크립트는 오일러 샘플링 방법을 기반으로 하므로 반드시 사용해야 합니다.
샘플링 단계: 50-60. 이것은 스크립트의 디코드 단계 값과 매우 일치합니다. 그렇지 않으면 좋지 않은 시간을 보내게 됩니다. 이 데모에서는 50을 사용합니다.
CFG 스케일: 2 이하. 이 데모에서는 1.8을 사용합니다. (힌트, ui-config.json을 편집하여 "img2img/CFG Scale/step"을 .5 대신 .1로 변경할 수 있습니다.
잡음 제거 강도 - 이것은 이전 문서 가 말한 것과 반대로 중요합니다. 1로 설정합니다.
너비/높이 - 입력 이미지의 너비/높이를 사용합니다.
시드... 무시해도 됩니다. 역 오일러는 지금 이미지에 대한 노이즈를 생성하고 있습니다.
Decode cfg scale - 1보다 낮은 곳이 스위트 스폿입니다. 데모의 경우 1을 사용합니다.
디코딩 단계 - 위에서 언급했듯이 샘플링 단계와 일치해야 합니다. 데모의 경우 50, 더 자세한 이미지를 보려면 60으로 늘리는 것이 좋습니다.
위의 모든 사항이 입력되면 "생성"을 누르고 원본에 매우 가까운 결과를 얻을 수 있어야 합니다 .

스크립트가 원본 사진을 높은 정확도로 다시 생성하는지 확인한 후 프롬프트의 세부 정보를 변경할 수 있습니다. 원본의 변형이 크면 원본과 완전히 다른 구성의 이미지가 생성될 수 있습니다.

위의 설정과 아래 프롬프트를 사용한 예제 출력(빨간 머리/포니는 사진에 없음)

![](2022-10-20-13-12-00.png)

"푸른 머리에 웃는 여자." 공장. "갈색 머리에 찡그린 여자." 공장. "빨간 머리에 찡그린 여자." 공장. "빨간 머리에 찡그린 여자가 말을 타고 있다." 여성을 완전히 대체하는 것 같으며 이제 생강 조랑말이 있습니다.

user.css
user.cssNear 라는 파일을 webui.py만들고 커스텀 CSS 코드를 넣습니다. 예를 들어 갤러리를 더 크게 만듭니다.

#txt2img_gallery, #img2img_gallery{
    min-height: 768px;
}
알림.mp3
webui의 루트 폴더에 라는 이름의 오디오 파일 notification.mp3이 있으면 생성 프로세스가 완료되면 재생됩니다.
...
조정
CLIP 모델의 마지막 레이어 무시
이것은 설정의 슬라이더이며 CLIP 네트워크에 의한 프롬프트 처리를 얼마나 일찍 중지해야 하는지를 제어합니다.

더 자세한 설명:

CLIP은 프롬프트 텍스트를 숫자 표현으로 변환하는 매우 진보된 신경망입니다. 신경망은 이 수치 표현과 매우 잘 작동하므로 SD 개발자는 안정적인 확산의 이미지 생성 방법과 관련된 3가지 모델 중 하나로 CLIP을 선택했습니다. CLIP은 신경망이기 때문에 레이어가 많다는 의미입니다. 귀하의 프롬프트는 간단한 방법으로 디지털화되고 레이어를 통해 전달됩니다. 첫 번째 레이어 이후에 프롬프트의 숫자 표현을 얻고, 두 번째 레이어에 입력하고, 그 결과를 세 번째 레이어에 제공하는 등 마지막 레이어에 도달할 때까지 안정적으로 사용되는 CLIP의 출력입니다. 확산. 이것은 슬라이더 값 1입니다. 그러나 일찍 중지하고 마지막 레이어 다음의 출력을 사용할 수 있습니다. 즉 슬라이더 값은 2입니다. 더 일찍 중지하면

일부 모델은 이러한 종류의 조정으로 훈련되었으므로 이 값을 설정하면 해당 모델에서 더 나은 결과를 생성하는 데 도움이 됩니다.
